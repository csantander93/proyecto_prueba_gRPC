...
// Cargar el archivo .proto de order
...
function crearProducto() {
    const nuevoProducto = {
        codigo: 'cris123',
        nombre: 'Nombre del Producto',  // Nombre del producto
        talle: 'M',
        foto: 'url_a_la_foto',
        color: 'rojo',
        stock: 50,
        id_tienda: 1  // ID de la tienda
    };
...
// Llamada para crear un producto
crearProducto();
...
function createOrder() {
  ...
  const orderRequest = {
      ...
      orden_despacho: 'Dispatch001',
      // Convertir a formato YYYY-MM-DD HH:MM:SS sin el sufijo Z
      fecha_solicitud: now.toISOString().slice(0, 19).replace('T', ' '),
      fecha_recepcion: now.toISOString().slice(0, 19).replace('T', ' '), // Cambia según sea necesario
      items: [
          {
              codigo_articulo: 'cris123',
              color: 'rojo',
              talle: 'M',
              cantidad: 2
          },
  ...
  orderClient.CreateOrder(orderRequest, (error, response) => {
      if (error) {
